...
def Start():
    ...
    url = f"https://api.nike.com/product_feed/threads/v2?filter=language(en)&filter=marketplace(IL)&filter=channelId(d9a5bc42-4b9c-4976-858a-f159cf99c647)&filter=productInfo.merchProduct.styleColor({sku_test})"
                    ...
                    for item in item["productInfo"]:
                        try:
                            for size in item["skus"]:
                                id = size["id"]
                                localizedSize = size["countrySpecifications"][0]["localizedSize"]
                                for stock in item["availableSkus"]:
                                    if stock["id"] == id:
                                        availableSkus = stock["level"]
                                        array_size += f"{localizedSize} [{availableSkus}]\n"
                        except:
